...
class VLLMVersionManager:
    ...
    def list_available_versions(self) -> List[VLLMDefaultsVersion]:
        ...
        for file_path in self.defaults_dir.glob("v*.yaml"):
            ...
            try:
                ...
                total_defaults = sum(
                    len(section_defaults) for section_defaults in defaults_data.values()
                )
    ...
    def compare_versions(self, version1: str, version2: str) -> Dict[str, Any]:
        ...
        for key in all_keys:
            if key not in flat1:
                ...
                changed.append(
                    {"parameter": key, "old_value": flat1[key], "new_value": flat2[key]}
                )
            else:
    ...
    def get_version_info_summary(self) -> str:
        ...
        for i, version in enumerate(versions):
            ...
            lines.append(
                f"   Created: {version.creation_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
...
def main():
    ...
    if latest:
        ...
        try:
            ...
            for section, section_defaults in list(defaults.get("defaults", {}).items())[
                :2
            ]:
                print(f"   {section}:")
