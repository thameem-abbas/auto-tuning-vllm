...
def parse_postgres_url(database_url: str) -> Tuple[str, str, str, int, str, str]:
    ...
    if not user or not host or not database_name:
        raise ValueError(
            "PostgreSQL URL must include username, host, and database name"
        )
...
def create_database_if_not_exists(database_url: str) -> bool:
    ...
    user, password, host, port, database_name, base_url = parse_postgres_url(
        database_url
    )
...
def clear_study_data(
    ...
    result = {"success": False, "trials_deleted": 0, "logs_deleted": 0, "error": None}
    ...
    try:
        ...
        with psycopg2.connect(database_url) as conn:
            with conn.cursor() as cur:
                # Get study ID first
                cur.execute(
                    "SELECT study_id FROM studies WHERE study_name = %s", (study_name,)
                )
                ...
                cur.execute(
                    "SELECT COUNT(*) FROM trials WHERE study_id = %s",
                    (optuna_study_id,),
                )
                ...
                # Delete trials
                cur.execute(
                    "DELETE FROM trials WHERE study_id = %s", (optuna_study_id,)
                )

                # Delete study
                cur.execute(
                    "DELETE FROM studies WHERE study_id = %s", (optuna_study_id,)
                )
        ...
        if clear_logs:
            ...
            with psycopg2.connect(logs_db_url) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "SELECT COUNT(*) FROM trial_logs WHERE study_name = %s",
                        (study_name,),
                    )
                    ...
                    # Delete trial logs
                    cur.execute(
                        "DELETE FROM trial_logs WHERE study_name = %s", (study_name,)
                    )

                    result["logs_deleted"] = logs_count

                    logger.info(
                        f"Deleted {logs_count} log entries for study '{study_name}'"
                    )
