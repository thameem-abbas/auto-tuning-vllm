...
from auto_tune_vllm.utils import ArgumentType, VLLMCLIParser
...
def main():
    ...
    defaults_data = parser.export_defaults_yaml(
        str(defaults_file), optimization_sections
    )
    print(f"💾 Saved vLLM defaults to: {defaults_file}")

    # Show some examples of extracted defaults
    total_defaults = sum(
        len(section_defaults)
...
def generate_example_config(parser: VLLMCLIParser, sections: list) -> dict:
    ...
    config = {
        ...
        "optimization": {
            ...
            "objectives": [{"metric": "requests_per_second", "direction": "maximize"}],
            "sampler": "tpe",
            "n_trials": 100,
        },
        "benchmark": {
        ...
        "logging": {"file_path": "/tmp/auto-tune-vllm-logs", "log_level": "INFO"},
        "parameters": {},
    }
    ...
    for section in sections:
        ...
        for param_name in interesting_params[section]:
            if param_name in section_args:
                ...
                if (
                    arg.arg_type == ArgumentType.INTEGER
                    ...
                    arg.arg_type == ArgumentType.FLOAT and arg.default_value is not None
                ):
                    param_config.update(
                        {
