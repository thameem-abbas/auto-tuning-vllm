...
from src.serving.optimization import (
    p95_latency_objective_function_DEPRECATED,
    run_parallel_trials,
)
from src.serving.run_baseline import run_baseline_test
from src.serving.utils import save_config_to_study, validate_huggingface_model
...
def build_grid_search_space(config):
    ...
    for param_name, param_config in parameters.items():
        ...
        if "options" in param_config:
            ...
            print(
                f"DEBUG: Options parameter {param_key}: {len(values)} values {values}"
            )
            ...
            total_combinations *= len(values)
            print(
                f"DEBUG: Range parameter {param_key}: {len(values)} values from {start} to {end} step {step}"
            )
            ...
            if isinstance(levels, list):
                ...
                print(
                    f"DEBUG: Level parameter {param_key}: {len(levels)} values {levels}"
                )
        else:
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["config", "p95_latency"],
        default="config",
        help="Optimization mode: config (use vllm_config.yaml) or p95_latency (minimize p95 latency)",
    )
    parser.add_argument(
        "--model", type=str, help="HuggingFace model name (default: Qwen/Qwen3-32B-FP8)"
    )
    parser.add_argument(
        "--max-seconds",
        type=int,
    ...
    parser.add_argument(
        "--n-trials", type=int, help="Number of optimization trials (overrides config)"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    for concurrency in concurrency_levels:
        ...
        baseline_metrics = run_baseline_test(
            ...
            STUDY_ID,
            concurrency,
            gpu_id=args.baseline_gpu,
        )

        if baseline_metrics is not None:
            baseline_results.append(
                {"concurrency": concurrency, "metrics": baseline_metrics}
            )
            print(
                f"Concurrency {concurrency} - Performance: {baseline_metrics['output_tokens_per_second']:.2f} output tokens/second"
            )
            print(
                f"Concurrency {concurrency} - Latency: {baseline_metrics['request_latency']:.2f} ms"
            )
        else:
    ...
    if args.dataset:
        ...
        print(f"Output tokens: {args.output_tokens}")
    n_trials = (
        args.n_trials if args.n_trials else optimization_config.get("n_trials", 200)
    )
    print(f"Number of trials: {n_trials}")
    print("=" * 50)
    ...
    if optimization_approach == "multi_objective":
        ...
        print(
            f"Using multi-objective optimization (throughput vs latency) with {sampler_name} sampler"
        )
    ...
    print(
        f"Using dynamic GPU scheduling with {sampler_name} sampler on GPUs: {gpu_ids}"
    )
    ...
    all_available, unavailable_ports, required_ports = (
        check_all_ports_available_for_study(gpu_ids, args.start_port)
    )
    print(
    ...
    if optimization_approach == "multi_objective":
        ...
        if baseline_metrics is not None:
            improvement = (
                (study.best_trial.value - baseline_metrics["output_tokens_per_second"])
                / baseline_metrics["output_tokens_per_second"]
            ) * 100
