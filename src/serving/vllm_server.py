...
def build_vllm_command(model_name, port, candidate_flags):
    ...
    while i < len(candidate_flags):
        ...
        if flag == "--enable-chunked-prefill" or flag.startswith(
    ...
    if (
        max_partial_prefills_value is not None
        and max_partial_prefills_value > 1
        and not has_enable_chunked_prefill
    ):
...
def start_vllm_server(
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to start vLLM server: {e.strerror}\nCommand: {' '.join(cmd)}"
        )
    ...
    while True:
        ...
        if elapsed > timeout:
            proc.kill()
            raise TimeoutError(
                f"vLLM server did not start within {timeout / 1000} seconds"
            )
...
def stop_vllm_server(proc, grace_period=15):
    ...
    if not killed and proc.poll() is None:
        print(
            f"ERROR: Process {proc.pid} could not be terminated - manual cleanup may be required"
        )
...
def query_vllm_server(port, prompt, max_retries=3, retry_delay=1):
    ...
    for attempt in range(max_retries):
        ...
        if attempt < max_retries - 1:
            print(
                f"Attempt {attempt + 1} failed: {last_error}. Retrying in {retry_delay}s..."
            )
            time.sleep(retry_delay)

    raise RuntimeError(
        f"Failed to query vLLM server after {max_retries} attempts. Last error: {last_error}"
    )
...
def cleanup_zombie_vllm_processes(ports_to_check=None):
    ...
    try:
        ...
        if not ports_in_use:
            print(
                f"All specified ports {ports_to_check} are available - no cleanup needed"
            )
            return
