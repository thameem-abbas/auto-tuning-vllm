...
def run_baseline_test(
    ...
    vllm_log_file = os.path.join(
        vllm_logs_dir,
        f"vllm_server_logs_{study_id}.baseline.concurrency_{concurrency}.log",
    )
    guidellm_log_file = os.path.join(
        guidellm_logs_dir,
        f"guidellm_logs_{study_id}.baseline.concurrency_{concurrency}.log",
    )
    ...
    baseline_flags = [
        "--max-num-seqs",
        str(concurrency),  # Limit concurrent sequencesoverhead
    ]

    vllm_cmd = build_vllm_command(
        model_name=model, port=port, candidate_flags=baseline_flags
    )
    vllm_proc = start_vllm_server(vllm_cmd, log_file=vllm_log_file, env=env)

    bench_file = os.path.join(
        study_dir, f"benchmarks_{study_id}.baseline.concurrency_{concurrency}.json"
    )
