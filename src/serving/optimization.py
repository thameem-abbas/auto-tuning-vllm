...
from src.serving.benchmarking import parse_benchmarks, run_guidellm
...
def run_single_trial_DEPRECATED(
    trial,
    model=None,
    max_seconds=None,
    prompt_tokens=None,
    ...
    vllm_log_file = os.path.join(
        vllm_logs_dir, f"vllm_server_logs_{study_id}.{trial_id}.log"
    )
    guidellm_log_file = os.path.join(
        guidellm_logs_dir, f"guidellm_logs_{study_id}.{trial_id}.log"
    )
    ...
    vllm_cmd = build_vllm_command(
        model_name=model, port=port, candidate_flags=candidate_flags
    )
    vllm_proc = start_vllm_server(vllm_cmd, log_file=vllm_log_file)

    bench_file = os.path.join(study_dir, f"benchmarks_{study_id}.{trial_id}.json")
    ...
    try:
        ...
        if dataset:
            ...
            print(
                f"Using synthetic data: {prompt_tokens} prompt tokens, {output_tokens} output tokens"
            )
...
# DEPRECATED: This function is no longer used - all workflows now use run_parallel_trials
def multi_objective_function_DEPRECATED(
    trial,
    model=None,
    max_seconds=None,
    prompt_tokens=None,
    ...
    try:
        metrics = run_single_trial_DEPRECATED(
            trial,
            model,
            max_seconds,
            prompt_tokens,
        ...
        print(
            f"Error during multi-objective trial {trial.number}:",
            str(e),
            file=sys.stderr,
        )
        import traceback
...
def objective_DEPRECATED(
    ...
    try:
        ...
        print(
            f"DEBUG: Trial {trial.number}, Metric value to return: {metrics['output_tokens_per_second']}"
        )
...
# DEPRECATED: This function is no longer used - all workflows now use run_parallel_trials
def p95_latency_objective_function_DEPRECATED(
    trial,
    model,
    max_seconds,
    prompt_tokens,
    ...
    try:
        ...
        print(
            f"Error during p95 latency trial {trial.number}:", str(e), file=sys.stderr
        )
        import traceback
...
def run_single_trial_parallel(
    ...
    trial_id = trial.number
    vllm_log_file = os.path.join(
        vllm_logs_dir, f"vllm_server_logs_{study_id}.{trial_id}_gpu{gpu_id}.log"
    )
    guidellm_log_file = os.path.join(
        guidellm_logs_dir, f"guidellm_logs_{study_id}.{trial_id}_gpu{gpu_id}.log"
    )
    ...
    bench_file = os.path.join(
        study_dir, f"benchmarks_{study_id}.{trial_id}_gpu{gpu_id}.json"
    )
    ...
    try:
        ...
        interval = vllm_config["settings"].get(
            "trial_interval", 10
        )  # Shorter interval for parallel
    ...
    def parallel_objective_wrapper(gpu_id, trial):
        ...
        try:
            ...
            if hasattr(study, "directions") and len(study.directions) > 1:
                ...
                latency = float(metrics["request_latency"])
                print(
                    f"[GPU {gpu_id}] Trial {trial.number}: {tokens_per_second:.2f} tokens/s, {latency:.2f}ms latency"
                )
                ...
                print(
                    f"[GPU {gpu_id}] Trial {trial.number}: {tokens_per_second:.2f} tokens/s"
                )
            ...
            if "CUDA out of memory" in error_msg or "OutOfMemoryError" in error_msg:
                print(
                    f"[GPU {gpu_id}] OOM detected - GPU memory exhausted, moving to next trial"
                )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(gpu_ids)) as executor:
        while completed_trials + failed_trials < n_trials:
            ...
            for gpu_id in gpu_ids:
                ...
                if gpu_state["status"] == "running" and gpu_state["future"]:
                    if gpu_state["future"].done():
                        try:
                            ...
                            print(
                                f"[GPU {gpu_id}] Trial {gpu_state['trial_id']} completed successfully, value: {result:.2f}"
                            )
                        except Exception as e:
                            print(
                                f"[GPU {gpu_id}] Trial {gpu_state['trial_id']} failed: {e}"
                            )
                ...
                if (
                    gpu_state["status"] == "idle"
                    and completed_trials + failed_trials < n_trials
                ):
                    try:
                        trial = study.ask()
                        ...
                        future = executor.submit(
                            parallel_objective_wrapper, gpu_id, trial
                        )
                        ...
                        print(
                            f"[GPU {gpu_id}] Assigned trial {trial.number} (Progress: {completed_trials + failed_trials}/{n_trials})"
                        )
            ...
            if gpu_states[gpu_id]["future"]:
                try:
                    ...
                    if gpu_states[gpu_id]["trial_obj"]:
                        ...
                        print(
                            f"[GPU {gpu_id}] Final trial {gpu_states[gpu_id]['trial_id']} completed, value: {result:.2f}"
                        )
                except Exception as e:
    ...
    if len(study.trials) > 0:
        try:
            ...
            for i, trial in enumerate(study.best_trials[:5]):
                ...
                print(
                    f"  Solution {i + 1}: Throughput={throughput:.2f} tokens/s, Latency={latency:.2f}ms"
                )
